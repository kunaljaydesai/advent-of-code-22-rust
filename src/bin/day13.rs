use std::{borrow::Borrow, cell::RefCell, io::BufRead, rc::Rc};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(line) = lines.next() {
        let left_str = line.ok().unwrap();
        let left_vec_int = VecInt::from_str(left_str).unwrap();
        let right_str = lines.next().unwrap().ok().unwrap();
        let right_vec_int = VecInt::from_str(right_str).unwrap();
        let order = VecInt::is_in_right_order(left_vec_int, right_vec_int);
        if matches!(order, CorrectOrder::True) {
            score += idx;
        } else if matches!(order, CorrectOrder::Continue) {
            panic!("Error...");
        }
        idx += 1;
        lines.next();
